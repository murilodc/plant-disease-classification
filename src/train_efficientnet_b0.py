"""Fine-tune and evaluate ImageNet-pretrained EfficientNet-B0 on PlantVillage."""
...
from efficientnet_b0_model import (
    DEFAULT_WEIGHTS,
...
def train_efficientnet_b0(
    ...
    """Fine-tune EfficientNet-B0 and evaluate its best validation checkpoint."""
    set_seed(seed)
    device = get_device()
    metadata_csv = Path(metadata_csv)
    image_root = Path(image_root)
    output_dir = Path(output_dir)
    checkpoint_path = output_dir / "efficientnet_b0_best.pth"
    ...
    preprocessing = validate_efficientnet_b0_preprocessing(DEFAULT_WEIGHTS)
    ...
    # The shared infrastructure may construct test, but no routine below sees it.
    train_validation_loaders = {
        split: dataloaders[split] for split in ("train", "validation")
    }

    model = create_efficientnet_b0(num_classes=num_classes, weights=DEFAULT_WEIGHTS)
    if not all(parameter.requires_grad for parameter in model.parameters()):
        raise RuntimeError(
            "Todos os parametros da EfficientNet-B0 devem estar treinaveis."
        )
    ...
    experiment_metadata.update(
        {
            ...
            "weights": "ImageNet",
            "torchvision_weights_api": "EfficientNet_B0_Weights.DEFAULT",
            "torchvision_weights_resolved": efficientnet_b0_weights_name(
                DEFAULT_WEIGHTS
            ),
    ...
    architecture_summary = describe_efficientnet_b0(model, DEFAULT_WEIGHTS)
    ...
    print(model)
    print(f"Pesos: {efficientnet_b0_weights_name(DEFAULT_WEIGHTS)}")
    ...
    history_path = save_history(
        history,
        output_dir / "efficientnet_b0_history.csv",
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=project_root / "results" / "efficientnet_b0",
    )

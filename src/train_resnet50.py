"""Fine-tune and evaluate ImageNet-pretrained ResNet50 on PlantVillage."""
...
from plantvillage_pytorch import IMAGE_SIZE, create_dataloaders
...
def restore_best_checkpoint(
    ...
    """Restore a saved best checkpoint and move the model to ``device``."""
    ...
    if expected_best_epoch is not None and checkpoint_epoch != expected_best_epoch:
        raise RuntimeError(
            "O checkpoint salvo nao corresponde a melhor epoca: "
            f"esperada={expected_best_epoch}, encontrada={checkpoint_epoch}."
        )
...
def train_resnet50(
    ...
    """Fine-tune ResNet50 and evaluate only the best model on validation."""
    set_seed(seed)
    device = get_device()
    ...
    # The existing split CSV is reused unchanged. A test DataLoader may be
    # constructed by the shared infrastructure, but it is never passed to any
    # training, checkpoint-selection, inference or metrics routine below.
    dataloaders = create_dataloaders(
        ...
        transform_factory=build_resnet50_transforms,
    )
    train_validation_loaders = {
        split: dataloaders[split] for split in ("train", "validation")
    }

    model = create_resnet50(num_classes=num_classes, weights=DEFAULT_WEIGHTS)
    if not all(parameter.requires_grad for parameter in model.parameters()):
        raise RuntimeError("Todos os parametros da ResNet50 devem estar treinaveis.")
    ...
    experiment_metadata.update(
        {
            ...
            "weights": "ImageNet",
            "torchvision_weights_api": "ResNet50_Weights.DEFAULT",
            "torchvision_weights_resolved": resnet50_weights_name(DEFAULT_WEIGHTS),
            ...
            "preprocessing": {
                ...
                "normalization_source": resnet50_weights_name(DEFAULT_WEIGHTS),
                "mean": list(weights_transform.mean),
                "std": list(weights_transform.std),
                "augmentation_policy": "same_as_baseline",
            },
        }
    )
    ...
    print(model)
    print(f"Pesos: {resnet50_weights_name(DEFAULT_WEIGHTS)}")
    ...
    architecture_summary.update(describe_resnet50(model, DEFAULT_WEIGHTS))
    save_experiment_metadata(architecture_summary, architecture_path)
    ...
    history_path = save_history(history, output_dir / "resnet50_history.csv")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=project_root / "results" / "resnet50",
    )
